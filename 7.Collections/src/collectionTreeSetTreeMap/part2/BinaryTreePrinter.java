...
public class BinaryTreePrinter {
    ...
    public static <T extends Comparable<?>> void printNode(MyTreeMap.Entry<T> root) {
        int maxLevel = BinaryTreePrinter.maxLevel(root);

        printNodeInternal(List.of(root), 1, maxLevel);
    }

    private static <T extends Comparable<?>> void printNodeInternal(List<MyTreeMap.Entry<T>> nodes, int level, int maxLevel) {
        ...
        List<MyTreeMap.Entry<T>> newNodes = new ArrayList<MyTreeMap.Entry<T>>();
        for (MyTreeMap.Entry<T> node : nodes) {
            if (node != null) {
    ...
    private static <T extends Comparable<?>> int maxLevel(MyTreeMap.Entry<T> node) {
        if (node == null)
            return 0;
